...
|Function|Purpose|
|--------|-------|
|beforeTest|Is invoked each time a new Test Case is executed. If it's marked as Ignored, this won't execute.|
|afterTest|Is invoked when a Test Case has finished. This includes when a test case is passes (is successful), or fails (errors).|
